...
class MyAttacker(BaseAttack):
    ...
    def attack(self, ori_features: sp.csr_matrix, ori_adj: sp.csr_matrix, labels: np.ndarray,
        ...
        removed = [x for x in range(ori_adj.shape[0]) if row[x] == 1 and labels[x] == labels[target_node]]

        added = [x for x in idx_train if labels[x] != labels[target_node]]
        added = np.random.permutation(added)
        added = added[: n_perturbations - len(removed)]

        print(f'{removed=}, {added=}')

        modified_adj[target_node, removed] = 0
        modified_adj[removed, target_node] = 0
        modified_adj[target_node, added] = 1
        modified_adj[added, target_node] = 1
